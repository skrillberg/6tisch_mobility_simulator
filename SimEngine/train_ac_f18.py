...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(scope):
        ...
        #final output layer
        layer = tf.layers.dense(layer,output_size,activation=output_activation)
...
class Agent(object):
    ...
    def define_placeholders(self):
        ...
        #print("in define placehollders",sy_ob_no)
        if self.discrete:
            sy_ac_na = tf.placeholder(shape=[None], name="ac", dtype=tf.int32) 
            #print("sy_ac_na defined",sy_ac_na)
        else:
            sy_ac_na = tf.placeholder(shape=[None, self.ac_dim], name="ac", dtype=tf.float32) 
            #print("sy_ac_na defined",sy_ac_na)
        # YOUR CODE HERE
        sy_adv_n = tf.placeholder(shape=[None],name="adv",dtype=tf.float32)
        #print("sy_adv_n defined",sy_adv_n)
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            # YOUR_CODE_HERE
            print("in forward pass ", self.ac_dim)
            sy_logits_na = build_mlp(sy_ob_no,self.ac_dim,"forward_pass",self.n_layers,self.size)
            return sy_logits_na
        else:
            # YOUR_CODE_HERE
            sy_mean = build_mlp(sy_ob_no,self.ac_dim,"forward_pass_continuous",self.n_layers,self.size)
            sy_logstd = tf.get_variable("sy_logstd",shape=(self.ac_dim,),initializer=tf.initializers.constant(0)) #pass in 0
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            print(sy_logits_na)
            samples = tf.multinomial(logits=sy_logits_na,num_samples=1)
            print(samples)
            return samples[0]
        else:
            sy_mean, sy_logstd = policy_parameters
            # YOUR_CODE_HERE
            print(tf.shape(sy_mean))
            normal_tensor = tf.random_normal(shape=tf.shape(sy_mean))
            print(tf.shape(normal_tensor))
            sy_logtile = tf.reshape(tf.tile(sy_logstd, [tf.shape(sy_mean)[0]]),tf.shape(sy_mean))
            print(tf.shape(sy_logtile))

            #probs = tf.contrib.distributions.MultivariateNormalDiag(
             #   loc=sy_mean,
              #  scale_diag=tf.exp(sy_logstd))
            #sy_sampled_ac = probs.sample()
            sy_sampled_ac = normal_tensor*tf.exp(sy_logstd) + sy_mean
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            sy_logits_na = policy_parameters
            probs = tf.contrib.distributions.Categorical(logits=sy_logits_na)
 
            # YOUR_CODE_HERE
            sy_logprob_n = probs.log_prob(sy_ac_na)
            print(sy_logprob_n)
            print(sy_ac_na)
    ...
    def sample_trajectory(self, env, animate_this_episode):
        ...
        while True:
            ...
            obs.append(ob)
            #raise NotImplementedError
            ac,params = self.sess.run([self.sy_sampled_ac, self.policy_parameters],feed_dict={self.sy_ob_no : np.array(ob).reshape(1,ob.shape[0]) })
            ac = ac[0]
            acs.append(ac)
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # YOUR CODE HERE
        #print(len(ob_no),len(ob_no[0]))
        
        v_prime = self.sess.run(self.critic_prediction,feed_dict={self.sy_ob_no : next_ob_no})
        v_curr = self.sess.run(self.critic_prediction,feed_dict={self.sy_ob_no : ob_no})
        #print(v_prime-v_curr)
        adv_n = re_n + self.gamma*v_prime - v_curr - np.array(terminal_n)*v_prime*self.gamma

        #for i in range(0,len(ob_no)):
        #    v_curr = self.session.run(self.critic_prediction,feed_dict={self.sy_ob_no : ob_no[i]})
        #    if terminal_n[i]:
        #        adv_n.append( re_n[i] - v_curr)
        #    else:
        #        v_prime = self.session.run(self.critic_prediction,feed_dict={self.sy_ob_no : next_ob_no[i]})
        #        adv_n.append(re_n[i] +self.gamma*v_prime - v_curr)
   
        #print(adv_n)
        if self.normalize_advantages:
            #adv_n = sklearn.preprocessing.scale(adv_n,axis=0,with_mean=True,with_std=True)
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # otherwise the values will grow without bound.
        # YOUR CODE HERE
        for target_update in range(0,self.num_target_updates):
            v_prime = self.sess.run(self.critic_prediction,feed_dict={self.sy_ob_no : next_ob_no})
            #print(terminal_n)
            y = np.array(re_n) + self.gamma*(v_prime-(v_prime*terminal_n))
            for step in range(0,self.num_grad_steps_per_target_update):
                something, loss = self.sess.run([self.critic_update_op,self.critic_loss],feed_dict={self.sy_target_n : y,
                                                                    self.sy_ob_no : ob_no})
                #print( loss)
        ###
        #raise NotImplementedError
...
def train_AC(
    ...
    for itr in range(n_iter):
        ...
        adv_n = agent.estimate_advantage(ob_no=ob_no, next_ob_no=next_ob_no, re_n=re_n, terminal_n=terminal_n)
        # (3) use the estimated advantage values to update the actor, by calling agent.update_actor
        agent.update_actor(ob_no=ob_no, ac_na=ac_na, adv_n=adv_n)
        # YOUR CODE HERE
        #raise NotImplementedError
